...
instance Pretty Defn where
  pretty Axiom{..} = "Axiom {" <?> vcat
    [ "axATPRole =" <?> pshow axTPTPRole
    , "axHints   =" <?> pshow axTPTPHints
    ] <?> "}"
  ...
  pretty Datatype{..} =
    "Datatype {" <?> vcat
      ...
      , "dataATPRole    =" <?> pshow dataTPTPRole ] <?> "}"
  pretty Record{..} =
    "Record {" <?> vcat
